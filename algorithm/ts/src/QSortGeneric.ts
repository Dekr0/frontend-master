...
function partition<T>(low: number, high: number, arr: Array<T>, compareFn: (a: T, b: T) => boolean): number {
...
function qsort<T>(low: number, high: number, arr: Array<T>, compareFn: (a: T, b: T) => boolean): void {
    if (low >= high) {
        return;
    }
